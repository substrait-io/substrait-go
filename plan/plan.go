...
import (
	...
	"github.com/substrait-io/substrait-go/v6/plan/internal"
...
func init() {
	if info, ok := debug.ReadBuildInfo(); ok {
		for _, dep := range info.Deps {
			if strings.HasPrefix(dep.Path, "github.com/substrait-io/substrait-go/v6") {
...
func (p *Plan) ToProto() (*proto.Plan, error) {
	uris, urns, decls := p.extensions.ToProto()
