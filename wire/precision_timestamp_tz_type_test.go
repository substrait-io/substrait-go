...
func TestPrecisionTimestampTzTypeToProto(t *testing.T) {
	...
	for _, precision := range allPrecision {
		for _, nullability := range allNullability {
			typ := types.NewPrecisionTimestampTzType(precision).WithNullability(nullability).(*types.PrecisionTimestampTzType)
			assertPrecisionTimestampTzProto(t, precision, nullability, *typ)
		}
	}
}

func assertPrecisionTimestampTzProto(t *testing.T, expectedPrecision types.TimePrecision,
	expectedNullability types.Nullability, toVerifyType types.PrecisionTimestampTzType) {
	...
	if diff := cmp.Diff(TypeToProto(&toVerifyType), expectedTypeProto, protocmp.Transform()); diff != "" {
		t.Errorf("PrecisionTimestampTzType proto didn't match, diff:\n%v", diff)
	}
	...
	if diff := cmp.Diff(FuncArgToProto(&toVerifyType), expectedFuncArgProto, protocmp.Transform()); diff != "" {
		t.Errorf("PrecisionTimestampTzType func arg proto didn't match, diff:\n%v", diff)
	}
}
