...
// TestLambdaBuilder_ScalarFuncWithLambdaArg tests building a scalar function
// that takes a lambda as one of its arguments — the exact pattern used by
// higher-order functions like TRANSFORM(array, lambda).
// This exercises the full pipeline: Lambda construction → FuncArg conversion → ScalarFunc build.
func TestLambdaBuilder_ScalarFuncWithLambdaArg(t *testing.T) {
	collection := ext.GetDefaultCollectionWithNoError()
	b := &expr.ExprBuilder{
		Reg: expr.NewEmptyExtensionRegistry(collection),
	}

	// Build a lambda: ($0: i64) -> $0  (identity function)
	params := &types.StructType{
		Nullability: types.NullabilityRequired,
		Types:       []types.Type{&types.Int64Type{Nullability: types.NullabilityNullable}},
	}
	lambdaExpr, err := b.Lambda(params,
		b.LambdaParamRef(expr.StructFieldRef{Field: 0}, 0),
	).Build()
	require.NoError(t, err, "Lambda build should succeed")
	require.NotNil(t, lambdaExpr)

	// Verify the lambda can be used as a FuncArg (this is how ScalarFunc receives it)
	funcArg := lambdaExpr.ToProtoFuncArg()
	require.NotNil(t, funcArg, "Lambda should produce a valid FuncArgument proto")
	require.NotNil(t, funcArg.GetValue(), "FuncArgument should have a value")
	require.NotNil(t, funcArg.GetValue().GetLambda(), "FuncArgument value should be a Lambda")

	t.Logf("✅ Lambda as FuncArg: %v", lambdaExpr.String())
}

// TestLambdaBuilder_LambdaBodyWithArithmeticOnParam tests building a lambda
// where the body performs arithmetic on the lambda parameter.
// This is the pattern for transform(array[1,2,3], x -> x + 1).
func TestLambdaBuilder_LambdaBodyWithArithmeticOnParam(t *testing.T) {
	collection := ext.GetDefaultCollectionWithNoError()
	b := &expr.ExprBuilder{
		Reg: expr.NewEmptyExtensionRegistry(collection),
	}

	addID := ext.ID{URN: ext.SubstraitDefaultURNPrefix + "functions_arithmetic", Name: "add"}
	...
	lambdaExpr, err := b.Lambda(params,
		b.ScalarFunc(addID).Args(
			b.LambdaParamRef(expr.StructFieldRef{Field: 0}, 0),
			b.Wrap(expr.NewLiteral(int64(1), false)),
		),
	).Build()
	require.NoError(t, err, "Lambda with add($0, 1) should build successfully")
	require.NotNil(t, lambdaExpr)

	// Verify the body is a ScalarFunction
	bodyFunc, ok := lambdaExpr.Body.(*expr.ScalarFunction)
	require.True(t, ok, "Body should be a ScalarFunction, got %T", lambdaExpr.Body)
	require.Equal(t, 2, bodyFunc.NArgs(), "add should have 2 arguments")

	// Verify the first arg is a FieldReference with LambdaParameterReference root
	firstArg := bodyFunc.Arg(0).(expr.Expression)
	fieldRef, ok := firstArg.(*expr.FieldReference)
	require.True(t, ok, "First arg should be FieldReference, got %T", firstArg)
	_, isLambdaRef := fieldRef.Root.(expr.LambdaParameterReference)
	require.True(t, isLambdaRef, "Root should be LambdaParameterReference")

	// Verify proto round-trip
	lambdaProto := lambdaExpr.ToProto()
	require.NotNil(t, lambdaProto.GetLambda(), "Proto should have Lambda")
	require.NotNil(t, lambdaProto.GetLambda().GetBody(), "Lambda proto should have body")

	t.Logf("✅ Lambda with arithmetic: %s", lambdaExpr.String())
}

// TestLambdaBuilder_LambdaProtoRoundTripWithParamRef tests that a lambda
// with parameter references survives a full proto round-trip.
func TestLambdaBuilder_LambdaProtoRoundTripWithParamRef(t *testing.T) {
	b := &expr.ExprBuilder{}

	// Build: ($0: i32, $1: string) -> $1
	params := &types.StructType{
		Nullability: types.NullabilityRequired,
		Types: []types.Type{
			&types.Int32Type{Nullability: types.NullabilityRequired},
	...
	original, err := b.Lambda(params,
		b.LambdaParamRef(expr.StructFieldRef{Field: 1}, 0),
	).Build()
	require.NoError(t, err)

	// Convert to proto
	protoExpr := original.ToProto()
	require.NotNil(t, protoExpr.GetLambda())

	lambdaProto := protoExpr.GetLambda()
	// Verify parameter types in proto
	require.Len(t, lambdaProto.Parameters.Types, 2, "Should have 2 parameter types")

	// Verify the body is a field reference with lambda_parameter_reference root
	bodySelection := lambdaProto.Body.GetSelection()
	require.NotNil(t, bodySelection, "Body should be a field reference selection")
	require.NotNil(t, bodySelection.GetLambdaParameterReference(),
		"Body root should be lambda_parameter_reference")
	require.Equal(t, uint32(0), bodySelection.GetLambdaParameterReference().StepsOut,
		"StepsOut should be 0")
	require.Equal(t, int32(1), bodySelection.GetDirectReference().GetStructField().Field,
		"Should reference field 1 (the string parameter)")

	t.Logf("✅ Proto round-trip preserves lambda parameter reference")
}
