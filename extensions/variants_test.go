...
// customReturnTypeWithPreSetRef is a test helper that returns a UserDefinedType
// with a pre-set TypeReference to test that it doesn't get overwritten
type customReturnTypeWithPreSetRef struct {
	name        string
	preSetRef   uint32
	nullability types.Nullability
}

func (c *customReturnTypeWithPreSetRef) String() string {
	return fmt.Sprintf("u!%s", c.name)
}

func (c *customReturnTypeWithPreSetRef) ShortString() string {
	return fmt.Sprintf("u!%s", c.name)
}

func (c *customReturnTypeWithPreSetRef) SetNullability(n types.Nullability) types.FuncDefArgType {
	c.nullability = n
	return c
}

func (c *customReturnTypeWithPreSetRef) HasParameterizedParam() bool {
	return false
}

func (c *customReturnTypeWithPreSetRef) GetParameterizedParams() []interface{} {
	return nil
}

func (c *customReturnTypeWithPreSetRef) MatchWithNullability(ot types.Type) bool {
	return false
}

func (c *customReturnTypeWithPreSetRef) MatchWithoutNullability(ot types.Type) bool {
	return false
}

func (c *customReturnTypeWithPreSetRef) GetNullability() types.Nullability {
	return c.nullability
}

func (c *customReturnTypeWithPreSetRef) ReturnType([]types.FuncDefArgType, []types.Type) (types.Type, error) {
	return &types.UserDefinedType{
		Nullability:   c.nullability,
		TypeReference: c.preSetRef, // Pre-set to non-zero value
	}, nil
}

func (c *customReturnTypeWithPreSetRef) WithParameters([]interface{}) (types.Type, error) {
	return nil, fmt.Errorf("not implemented")
}

func TestUserDefinedTypeReferenceNotOverwritten(t *testing.T) {
	// Test that if TypeReference is already set (non-zero), it doesn't get overwritten
	registry := extensions.NewSet()
	testURN := "extension:org:item"
	testTypeName := "test_type"

	// Get the expected reference that would be set if TypeReference was 0
	expectedRef := registry.GetTypeAnchor(extensions.ID{URN: testURN, Name: testTypeName})

	// Create a custom return type that returns a UserDefinedType with a pre-set TypeReference
	// Use a different value to verify it doesn't get overwritten
	preSetRef := uint32(999)
	customReturnType := &customReturnTypeWithPreSetRef{
		name:        testTypeName,
		preSetRef:   preSetRef,
		nullability: types.NullabilityRequired,
	}
	...
	// Verify that the pre-set TypeReference was NOT overwritten
	assert.Equal(t, preSetRef, udResult.TypeReference,
		"TypeReference should not be overwritten when it's already set (non-zero)")
	assert.NotEqual(t, expectedRef, udResult.TypeReference,
		"TypeReference should remain as the pre-set value, not the registry anchor")
}

func TestUserDefinedTypeReferenceSetWhenZero(t *testing.T) {
	// Test that if TypeReference is 0, it gets set correctly
	registry := extensions.NewSet()
	testURN := "extension:org:item"
	testTypeName := "test_type"

	// Get the expected reference
	expectedRef := registry.GetTypeAnchor(extensions.ID{URN: testURN, Name: testTypeName})

	// Create a custom return type that returns a UserDefinedType with TypeReference = 0
	customReturnType := &customReturnTypeWithPreSetRef{
		name:        testTypeName,
		preSetRef:   0, // Zero value - should be set
		nullability: types.NullabilityRequired,
	}
	...
	// Verify that TypeReference was set correctly
	assert.Equal(t, expectedRef, udResult.TypeReference,
		"TypeReference should be set when it's 0")
}
